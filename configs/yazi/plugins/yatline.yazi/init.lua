...
local function utf8sub(s, i, j)
  ...
  local function chars(_s, no_subs)
    ...
    return coroutine.wrap(function() return map(coroutine.yield) end)
  end
...
function Yatline.string.get:tab_num_files() return tostring(#cx.active.current.files) end
...
function Yatline.string.get:date(format) return tostring(os.date(format)) end
...
function Yatline.line.create(line, component_type) return line end
...
local function config_components(section_a_components, section_b_components, section_c_components, in_side)
  ...
  local section_a_line_components =
    config_components_separators(section_a_components, ComponentType.A, in_side, num_section_b_components, num_section_c_components)
  local section_b_line_components =
    config_components_separators(section_b_components, ComponentType.B, in_side, num_section_b_components, num_section_c_components)
  local section_c_line_components =
    config_components_separators(section_c_components, ComponentType.C, in_side, num_section_b_components, num_section_c_components)
...
return {
  setup = function(_, config)
    ...
    Progress.partial_render = function(self)
      ...
      return {
        gauge:percent(percent):label(ui.Span(string.format("%3d%%, %d left", percent, left)):style(THEME.status.progress_label)),
      }
    end
